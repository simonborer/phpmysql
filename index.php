    ...
    <div class="grid-container">
        ...
        <div class="grid-x">
            <div class="cell large-8 large-offset-0 medium-10 medium-offset-1">
              <?php
              // this will generate a call out message for status messages
              if(isset($_GET['p'])):
                $status = $_GET['p'];
                if($status == "success"):
                  echo '<p class="callout success" data-closable="slide-out-right">Employee Successfully Added!</button></p>';
                else:
                  echo '<p class="callout alert" data-closable="slide-out-right">Failed to add, an unknown error has occurred!</button></p>';
                endif;
              endif;
              ?>
                    ...
                    <tbody>
                        ...
                        if($result = mysqli_query($link, $sqlTimeTable)){
                            ...
                            if(mysqli_num_rows($result) > 0){
                                ...
                                // I changed the values to match my database
                                while($row = mysqli_fetch_array($result)){
                                    echo "<tr>";
                                    echo "<td>" . $row['name'] . "</td>";
                                    echo "<td>" . $row['shift_start_day'] . "</td>";
                                    echo "<td>" . $row['start_time'] . "</td>";
                                    echo "<td>" . $row['end_time'] . "</td>";
                                    echo "<td>" . $row['bakery_address'] . "</td>";
                                    echo "<td>" . $row['employee_description'] . "</td>";
                                    echo "</tr>";
                                }
                                // Free result set
         ...
         -->
            <?php
                ...
                if($_SERVER["REQUEST_METHOD"] == "POST"){
                    ...
                    $sql = "INSERT INTO employee_shift (employee_id, bakery_id, shift_start_day, shift_end_day, shift_start_time, shift_end_time) VALUES (?, ?, ?, ?, ?, ?)";
                    ...
                    if($stmt = mysqli_prepare($link, $sql)){
                        ...
                        mysqli_stmt_bind_param($stmt, "ssssss", $param_employee, $param_location, $param_start_day, $param_end_day , $param_start_time, $param_end_time);
                        ...
                        $param_start_day = $day;
                        $param_end_day = $day . "+ INTERVAL 1 WEEK";
                        ...
                        if(mysqli_stmt_execute($stmt)){
                            ...
                            // added a success query string so I can retrive it to show the message
                            ...
                            <label>Location
                                <select name="location" id="locationDatalist" required>
                                    ...
                                    if($result = mysqli_query($link, $sqlLocations)){
                                        if(mysqli_num_rows($result) > 0){
                                            while($row = mysqli_fetch_array($result)){
                                              // I changed the $row['bakery_id'], $row['bakery_name'], $row['bakery_address']
