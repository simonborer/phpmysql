<script>
  ...
  var formatDate = function formatDate() {
    ...
    var aYearAway = Object.create(null);
    aYearAway.now = [year, month, day].join("-");
    aYearAway.aYear = [year + 1, month, day].join("-");
  ...
  datePickers.forEach(function (datePicker) {
    datePicker.setAttribute("min", formatDate().now);
    datePicker.setAttribute("max", formatDate().aYear);
  });
...
<script>
  ...
  function goDark(){
    if(body.className == '' || body.className == null ){
      for(i=0;i<allLabels.length;i++){
        allLabels[i].className += 'dark';
      }

      for(i=0;i<allTD.length;i++){
        allTD[i].className += 'dark';
      }

      for(i=0;i<allTH.length;i++){
        allTH[i].className += 'dark';
      }
